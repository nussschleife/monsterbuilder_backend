;
DELETE FROM Sprache;
DELETE FROM Schadensart;
DELETE FROM Angriff;
DELETE FROM Trait;
DELETE FROM Ability_Score;
DELETE FROM Monster;

INSERT INTO Sprache (name) VALUES ('Draconic'),('Elven'),('Common'),('Infernal'),('Goblin');
INSERT INTO Trait (name) VALUES ('Elemental'),('Plant'),('Fey'),('Fiend'),('Zombie');
INSERT INTO Schadensart (name) VALUES('Piercing'),('Bludgeoning'),('Good'),('Psychic'),('Evil'),('Fire'),('Cold');
INSERT INTO Angriff (name, angriff_Modifikator, schaden_Modifikator, wuerfelanzahl, wuerfel, reichweite_ft) VALUES('testattacke', 2, 3, 4, 'W6', 30);
INSERT INTO Ability_Score (name, score_Name, score) VALUES ('DEXTERITY','DEXTERITY', 4);